...
void menu_principal(struct Professor professores[], struct Disciplina disciplinas[], struct Aluno alunos[]);
    void menu_principal_opcoes(int opcao, int cadastro_realizado); //mostrar opções com a devida marcação
...
void menu_principal_opcoes(int opcao, int cadastro_realizado) {
	//condição para indicar se cadastro foi realizado ou não
	if (cadastro_realizado == 0) {
	    switch (opcao) {
	        case 1:
	            titulo();
		...
		switch (opcao){
			case 1:
				do {
					...
					printf("        CADASTRAR\n");
					printf("\n");
					printf("        Cadastrar novamente? Todo o cadastro atual será perdido.\n\n");
					printf("            -> SIM\n");
					printf("               NÃO");
...
void relatorio_indisponivel() {
	...
	printf("        RELATÓRIO\n");
	printf("\n");
	printf("            Indisponível! Realize o CADASTRO primeiro.");
...
void relatorio_1 (struct Disciplina disciplinas[], struct Aluno alunos[], int num_alunos) {
    ...
    //atribuição do tamanho do maior nome de aluno, se estiver matriculado nesta disciplina
    for (i = 0; i < num_alunos; i++){
...
//basicamente uma cópia do relatório_1
void relatorio_2(struct Disciplina disciplinas[], struct Aluno alunos[], int num_alunos) {
...
void ajuda() {
	...
	do {
		...
		printf("            além de 1 até 15 alunos.\n");
		printf("            - O cadastro é realizado de uma só vez.  Então, i-\n");
		printf("            niciado o processo, ele só termina após o cadastro\n");
		printf("            do último aluno.\n\n");
		printf("            RELATÓRIO\n\n");
		printf("            -  Após o CADASTRO,  pode-se gerar 2 relatórios de\n");
		printf("            alunos matriculados nas duas disciplinas.\n");
		printf("            O  relatório  da primeira disciplina será ordenado\n");
		printf("            por idade, na  ordem  crescente;  já o segundo, na\n");
		printf("            decrescente.\n\n");
